...
';
			...
			SELECT * FROM #LogInfoByFile;';
				...
				BEGIN
					...
					SELECT 'SELECT * FROM #LogFilesVLFs;' AS RunningQuery, * FROM #LogFilesVLF ORDER BY DB_Name;
				END
...
' ELSE '' END) + (CASE WHEN @forceExecute = 0 THEN 'USE ['+@c_DBName+']; ' ELSE '' END)+ 'DBCC SHRINKFILE (N'''+@c_FileName+''' , 0, TRUNCATEONLY);'
					...
					BEGIN -- Execute SHRINKFILE
						BEGIN TRY
							IF @verbose = 1 AND @_loopSQLText_BackupLog IS NOT NULL AND LEN(@_loopSQLText_BackupLog) > 0
							BEGIN
								PRINT 'Proceeding to Execute Backup Transaction Log';
								PRINT @_loopSQLText_BackupLog;
							END
							...
							BEGIN -- block for DbccShrinkFile
								...
								BEGIN
									SELECT	Q.RunningQuery, o.*
									FROM  (	SELECT 'select * from @T_DbccShrinkFile_Raw WHERE ID <= 3' AS RunningQuery ) Q
									LEFT OUTER JOIN
										  (SELECT * FROM @T_DbccShrinkFile_Raw WHERE ID <= 3 ) AS o
									ON		1 = 1;
								END						
								...
								SELECT * FROM #LogFilesVLFs as f inner join 
								IF EXISTS(SELECT * FROM @T_DbccShrinkFile AS o WHERE ((o.CurrentSize * 8.0)/1024) >= 8000)
								BEGIN
									SET @_errorOccurred = 1;
									SET @_errorMSG = '*** The log File '''+@c_FileName+''' did not Shrink. This could be due to many reasons. Kindly run below query to troubleshoot:-
...
';
									...
									SELECT	'Fail' AS Status
											...
											,@_loopSQLText_DbccShrinkFile AS TSQLCode;
								END
					...
					END -- Execute SHRINKFILE
								...
								BEGIN
									IF @verbose = 1
									BEGIN
										PRINT 'Proceeding to re-grow the Log file';
								...
								IF LEN(@_loopSQLText_AlterDbAutoGrowth) > 10
								BEGIN
									IF @verbose = 1
										PRINT 'Setting the File AutoGrowth: 
'+@_loopSQLText_AlterDbAutoGrowth;
...
'
					...
					ELSE
					BEGIN
						SELECT @_loopCounter=1, @_loopCounts=CEILING((@c_Size_mb/1024.0)/(1.0*@c_new_autogrowth));
						SET @_loopSQLText_AlterDbModifySize = '
...
'
						...
						BEGIN	-- Begin Block of Loop
							SET @_loopSQLText_AlterDbModifySize += 'ALTER DATABASE ['+@c_DBName+'] MODIFY FILE (NAME = N'''+@c_FileName+''''+(CASE WHEN @_loopCounter <> @_loopCounts THEN ', SIZE = '+CAST(@c_new_autogrowth*@_loopCounter AS VARCHAR(20))+'GB' ELSE '' END)+');
'
							SET @_loopCounter = @_loopCounter + 1;
...
/*	******************** Begin:	@help = 1 *****************************/';
		...
		FROM	(SELECT	'!~~~ Version ~~~~!' as [Parameter Name],'Information' as [Data Type],'3.7' as [Default Value],'Last Updated - 30/Apr/2019' as [Parameter Description]
					--
				UNION ALL
					--
